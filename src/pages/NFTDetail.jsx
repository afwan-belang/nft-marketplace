...
import { 
  ...
  X, CheckCircle, Wallet, ChevronRight, Loader2, ShieldCheck, AlertCircle 
} from 'lucide-react';
...
const NFTDetail = () => {
  ...
  // Tangkap data yang dikirim dari NFTCard
  ...
  // Data dinamis dengan fallback (jika user me-refresh halaman, state bisa hilang)
  const nftData = {
    title: navState?.title || (id ? id.replace(/-/g, ' ').toUpperCase() : 'UNKNOWN NFT'),
    author: navState?.author || "mary_jane",
    price: navState?.price || 2.45,
    usdPrice: `$${((navState?.price || 2.45) * 3500).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`,
    likes: navState?.likes || "12.4K",
    views: `${Math.floor(Math.random() * 50 + 10)}.${Math.floor(Math.random() * 9)}K`,
    image: navState?.image || "https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?q=80&w=1000&auto=format&fit=crop",
    description: `An exclusive digital manifestation of ${navState?.title || 'this artwork'}. This 1/1 artwork represents the genesis of the cyberpunk era in the Ethereum blockchain.`,
    accentColor: navState?.accentColor || "#a855f7",
    properties: [
      { type: "Background", value: "Deep Void", rarity: "8%" },
      { type: "Aura", value: "Neon", rarity: "12%" },
      { type: "Entity", value: "Cyber", rarity: "2%" }
    ]
  };
  ...
  const handleConfirmPurchase = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        // Pindahkan NFT ke profil user dan potong saldo via Zustand
        buyNFT(nftData);
        setTxState('success'); 
      }, 3000); 
    }, 2000); 
  ...
  return (
    <>
        ...
        <div className="flex flex-col lg:flex-row gap-6 md:gap-10">
          ...
          <div className="w-full lg:w-3/5 flex flex-col">
            ...
            <div className="glass-panel rounded-3xl p-5 md:p-6 border border-white/10 mb-6 bg-dark-bg/40">
              ...
              {/* ACTION BUTTONS (Dinamis berdasarkan kepemilikan) */}
              ...
              <div className="mt-4 border-t border-white/10 pt-4 hidden md:block">
                ...
                <LiveChart color={nftData.accentColor} />
              </div>
            </div>
      ...
      {/* =======================================================
          WEB3 TRANSACTION MODAL (CENTERED & HIGH BLUR)
          ======================================================= */}
      <AnimatePresence>
        {isModalOpen && (
              ...
              {txState === 'review' && (
                <div className="py-2">
                  ...
                  <Button 
                    ...
                    className={`w-full py-4 text-base md:text-lg font-bold transform-gpu ${balance < (nftData.price + 0.015) ? 'opacity-50 cursor-not-allowed' : 'shadow-[0_0_20px_rgba(138,43,226,0.4)]'}`} 
